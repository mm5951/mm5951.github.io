...
### Temperature range per weather station
```{r}
#MISSING : text label mutate(text_label = str_c("Tmax: ºC", max_temp, "\nTmin: ºC", min_temp))
...
Distribution of days with rain over time.
...
Count of days with snow distribution over time (per weather station).
...
## Temperature {.tabset .tabset-fade}
...
min_temp = noaa_df %>%
  ...
  plot_ly(
    x = ~month, y = ~min_temp, type = "scatter", mode = "markers",
    color = ~id, text = ~text_label, alpha = 0.5, colors = "viridis")
